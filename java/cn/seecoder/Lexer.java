...
public class Lexer {
    ...
    Lexer(ArrayList<String> my_token) {
        index = 0;
...
//--------------------------------下面都是加括号，复杂度是 O(n^2) ---------------------------------------
    ...
    Lexer subLexer(int left, int rihgt) {
        ArrayList<String>temp=new ArrayList<>();
        temp.addAll (my_token.subList(left,rihgt));
        return new Lexer(temp);
    }
    ...
    void connect(Lexer a){
        this.my_token.addAll(a.my_token);
        this.index=0;
    }

    Lexer full_parenthesis() {//好吧 这才是真正的加括号
        index=0;
        ...
        //else:
            ...
            if(getValue(1).equals(lambda)){
                Lexer a=this.subLexer(1,4);
                Lexer b=this.subLexer(4,this.lenth()-1);///
                ...
                Lexer a = this.subLexer(1, index);
                if(a.match(0)!=a.lenth()-1){
                    a.add_a_parenthesis();
                }
                Lexer b = this.subLexer(index, this.lenth() - 1);
